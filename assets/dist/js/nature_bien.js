function param() {
  ...
  $.ajax({
    type: "POST",
    url: base_local + "/config_app/propriete/",
    data: data,
    headers: {
      Authorization: "Bearer " + token,
    },
    success: function (response) {
      alert("type du Bien enregisté avec succèss");
      $("#inter_id").val("");
      $("#stat_id").val("s");
...
function getInterventionAndStatut() {
  var content = "";
  var url_go = base_local + "/config_app/propriete/";
  $.ajax({
    type: "GET",
    url: url_go,
    headers: {
        ...
        $("#tableInte").append(
          "<tr>\
            ...
            "</td>\
                        ...
                        <a onclick=getToEdit(" +
            elt["id"] +
            ')><i class="bi bi-pencil-square"style="color: rgb(0, 0, 0)"></i></a>&nbsp;<a href="#"><i class="bi bi-eye" style="color: rgb(136, 102, 119)"></i></a>\
                        </td>\
                    </tr>'
        );
        i++;
...
function editIntervention() {
  ...
  $.ajax({
    type: "PUT",
    url: base_local + "/config_app/propriete/" + id.toString(),
    data: data,
    headers: {
      Authorization: "Bearer " + token,
    },
    success: function (response) {
      alert("type de nature de bien modifié avec succèss");
      $("#inter_idE").val("");
      $("#stat_idE").val("s");
      $("#editForm").modal("hide");
      getInterventionAndStatut();
